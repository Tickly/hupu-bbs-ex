...
const nd = kw(), Lw = /* @__PURE__ */ tt({
  ...
  setup(e) {
    const t = $e([]), n = (r) => {
      Ka(".bbs-post-wrapper.light").find(".bbs-post-wrapper-content").find(".post-reply-list").each((d, p) => {
        const b = Ka(p).find(".bbs-admin-reply-post-container").data("admininfo"), { pid: C } = b, y = t.value.find((A) => A.pid === C);
        if (!y || !y.check_reply_info)
          return;
        const T = document.createElement("div");
        T.classList.add("todo-list", "todo-list-reply-dialog");
        const w = document.createElement("span");
        w.classList.add("todo-list-text", "bold"), w.innerHTML = `弹框查看评论(${y.check_reply_info.num})`, T.append(w), T.addEventListener("click", () => {
          Bh(r, C);
        }), Ka(p).find(".post-reply-list-operate").append(T);
      });
    };
    return Qt(() => {
